...
function Panel({
  ...
  return (
    <section
      className={`workspace-panel min-w-0 rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-5 shadow-[var(--shadow)] backdrop-blur-xl ${className}`}
    >
      <h3 className="m-0 text-[11px] font-extrabold uppercase tracking-[0.12em] text-[var(--muted)]">
        {title}
      </h3>
      {description && (
        <p className="mb-4 mt-2 text-xs leading-relaxed text-[var(--muted)]">
          {description}
        </p>
      )}
      {children}
...
export function TransformRulesPanel({
  ...
  return (
    ...
    >
      <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-[minmax(125px,.8fr)_minmax(145px,1fr)_minmax(145px,1fr)_auto]">
        <select
          value={ruleKind}
          onChange={(event) => {
          ...
          className="min-h-9 min-w-0 rounded-lg border border-[var(--line)] bg-[var(--surface-solid)] px-3 text-xs text-[var(--foreground)]"
        >
        ...
        <input
          ...
          className="min-h-9 min-w-0 rounded-lg border border-[var(--line)] bg-[var(--surface-solid)] px-3 text-xs text-[var(--foreground)] placeholder:text-[var(--muted)]"
        />
        {ruleKind === "map" ? (
          <select
            value={ruleValue}
        ...
        <button
          ...
          className="min-h-9 rounded-lg border border-[var(--foreground)] bg-[var(--foreground)] px-3 text-xs font-bold text-[var(--background)]"
        >
          + Add rule
        </button>
      </div>
      <div className="mt-4 flex min-h-7 flex-wrap gap-2 border-t border-[var(--line)] pt-3">
        {rules.length === 0 ? (
    ...
    >
      <div className="grid grid-cols-2 gap-2.5">
        {cards.map(([label, value]) => (
          <div
            key={label}
            className="min-w-0 rounded-xl border border-[var(--line)] bg-[color-mix(in_srgb,var(--surface-solid)_82%,var(--accent)_18%)] p-2.5"
          >
            <span className="block text-[9px] font-extrabold uppercase tracking-[0.08em] text-[var(--muted)]">
              {label}
            </span>
            <strong className="mt-1 block text-base leading-none text-[var(--foreground)]">
              {value}
            </strong>
          </div>
        ))}
      ...
      <button
        ...
        className="mt-3 min-h-9 rounded-lg border border-[var(--line)] px-3 text-xs font-semibold text-[var(--foreground)]"
      >
